...
"""
    ...
    def test_yfactor_live_target_callback_refreshes_displayed_target(self):
        app = self.make_app()
        updates = []
        ...
        class _YFactorSession(_FakeSession):
            ...
            def guarded_slew_to(self, azimuth, elevation, *args, **kwargs):
                updates.append((azimuth, elevation))
                kwargs["target_callback"](Position(0.0, 0.0, 10.0, 80.0))

            def read_position(self):
                return Position(0.0, 0.0, 12.0, 80.0)

        app.apply_target(TargetPosition("Sun", 10.0, 30.0))
        app.card_targets["East"] = TargetPosition("Cold Sky", 10.0, 80.0)

        app.refresh_yfactor_dwell_tracking(
            "East",
            _YFactorSession(),
            TargetPosition("Cold Sky", 10.0, 80.0),
            TargetPosition("Sun", 10.0, 30.0),
            app.card_targets["East"],
            lambda: TargetPosition("Cold Sky", 12.0, 80.0),
            lambda: TargetPosition("Sun", 12.0, 30.0),
        )
        app.process_events()

        self.assertEqual(updates, [(10.0, 80.0)])
        self.assertEqual(app.source_az.text(), "012.00")
        self.assertAlmostEqual(app.card_targets["East"].azimuth, 12.0)
        self.assertEqual(app.cards["East"].target.text(), "012.00 / 80.00")

    def test_tracking_fault_cleanup_does_not_override_yfactor_state(self):
        app = self.make_app()
        app.sessions = {"East": _FakeSession()}
        ...
        self.assertEqual(app.cards["East"].state.text(), "YFACTOR")

    def test_yfactor_handover_waits_for_existing_tracking_thread(self):
        app = self.make_app()
        ...
        app.tracking_kind = "moon"

        ok = app.stop_tracking_for_yfactor_handover(_Dialog())

        self.assertTrue(ok)
        self.assertTrue(thread.join_calls)
        self.assertIsNone(app.tracking_thread)
        self.assertEqual(app.tracking_kind, "")
